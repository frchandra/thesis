...
int main(){
    LogComponentEnable("Theta", LOG_LEVEL_INFO);
    ...
    for(int i = 0; i < 2; i++) { //TCP A
        ...
        OnOffHelper tcpHelper ("ns3::TcpSocketFactory", Address(InetSocketAddress (TcpinternetIpIfaces.GetAddress (1) /*receiver address (server address)*/, dlPort_tcp)));
        if(i > ATcpFlowNum-1){
            tcpHelper.SetConstantRate(DataRate("0Mb/s"), onOffPktSize);
        } else {
            tcpHelper.SetConstantRate(DataRate(onOffUpRate), onOffPktSize);
        }
        tcpHelper.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant="+ ofOnTime +"]"));
        tcpHelper.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant="+ ofOffTime +"]"));
    ...
    for(int i = 0; i < 2; i++) {
        OnOffHelper quicHelper ("ns3::QuicSocketFactory", Address(InetSocketAddress (QuicinternetIpIfaces.GetAddress (1 )  /*receiver address (server address)*/, dlPort_quic)));
        if(i > AQuicFlowNum-1){
            quicHelper.SetConstantRate(DataRate("0Mb/s"), onOffPktSize);
        } else {
            quicHelper.SetConstantRate(DataRate(onOffUpRate), onOffPktSize);
        }
        quicHelper.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant="+ ofOnTime +"]"));
        quicHelper.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant="+ ofOffTime +"]"));
