...
NS_LOG_COMPONENT_DEFINE("BaseModel");
...
class NodeStatistics{
  public:
    ...
    NodeContainer nodes;
    std::map<uint16_t, SignalNoiseDbm> signalNoise;

    NodeStatistics(NodeContainer nodes, std::string flowName);
    void SetPosition(NodeContainer nodeContainer, int stepSize);
    void AdvancePosition(NodeContainer nodeContainer, int stepsSize, int stepsTime);
    void Metrics();
...
NodeStatistics::NodeStatistics(NodeContainer nodes, std::string flowName): flowMonHelper(), asciiHelper(){
    this->nodes = nodes;
    monitor = flowMonHelper.Install(this->nodes);
    this->flowName = flowName;
    for(uint32_t i = 1; i <= (nodes.GetN()-1)*2 ; i++){
        std::string metricFileName = flowName + "flow-" + std::to_string(i) +  ".csv";
        std::ofstream metricOut(metricFileName, std::ios::app);
        metricOut << "dist,kbps,jitter_mils,plr,pdr,delay_mils,pkt_sent,pkt_rcv,pkt_loss,signal,noise,source,dest" << std::endl;
    }
}

void NodeStatistics::SetPosition(NodeContainer staNodeContainer, int stepSize){
    for (uint32_t i = 0; i < staNodeContainer.GetN(); i++) {
        Ptr<Node> node = staNodeContainer.Get(i);
...
void NodeStatistics::MonitorSnifferRxCallback(std::string context, Ptr<const ns3::Packet> packet, uint16_t channelFreqMhz, ns3::WifiTxVector txVector, ns3::MpduInfo aMpdu, ns3::SignalNoiseDbm sigNoi, uint16_t staId){
    this->signalNoise[staId] = sigNoi;
}

void NodeStatistics::AdvancePosition(NodeContainer staNodeContainer, int stepsSize, int stepsTime){
    NS_LOG_INFO("### ADVANCING: STEP " << stepItr << "; STA NODE_NAME: " << this->flowName << " ###");
    stepItr++;
    Metrics();
    SetPosition(staNodeContainer, stepsSize);
...
void NodeStatistics::Metrics(){
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin(); iter != stats.end(); ++iter){
        ...
        NS_LOG_UNCOND(" " << stepItr << "|kbps:"
                          << iter->second.rxBytes * 8.0/(iter->second.timeLastRxPacket.GetSeconds() - iter->second.timeFirstTxPacket.GetSeconds())/1024 << "|jitter_mils:"
        ...
        std::string fileName = flowName + "flow-" + std::to_string(iter->first) +  ".csv";
    ...
    for (auto i = signalNoise.begin(); i != signalNoise.end(); i++ ) {
        std::string signalFileName = flowName + "snr-"+ std::to_string(i->first) +".csv";
        std::ofstream outFile(signalFileName, std::ios::app);
        outFile << stepItr << ","
                << i->second.signal << ","
                << i->second.noise << std::endl;
    }
...
int main(){
    LogComponentEnable("BaseModel", LOG_LEVEL_INFO);
    ...
    Ptr<RateErrorModel> errorRateModel = CreateObject<RateErrorModel>();
    errorRateModel->SetAttribute("ErrorRate", DoubleValue(errorRate));
    apToGw.Get(1)->SetAttribute("ReceiveErrorModel", PointerValue(errorRateModel));
...
//    ApplicationContainer tcpServer = tcpSinkUp.Install(tcpServerNode.Get(0));
    //TCP onoff server - client downstream
    ApplicationContainer tcpClient;
    ...
    for(int i = 0; i < nTcp; i++){
        ...
        tcpOnoffDown.SetConstantRate(DataRate(onOffDataRate), onOffPktSize);
...
//    onoff.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant="+ ofOnTime +"]"));
//    onoff.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant="+ ofOffTime +"]"));
...
//    if(nTcp > 0){
//        tcpHttpServer.Start(Seconds(0.5));
//        tcpHttpClient.Start(Seconds(0.5));
//        tcpHttpServer.Stop(Seconds(simuTime));
//        tcpHttpClient.Stop(Seconds(simuTime));
//    }
    if(nQuic > 0){
...
//    }
    ...
    NodeStatistics nodeStat(
        NodeContainer(tcpServerNode.Get(0), wifiTcpStaNodes),
        "./" + folderName +  "/" + "TCP-");
